...
def find_circle(sess, graph, img):
    ...
    detected = sess.run([prediction], feed_dict={x: img, train_mode: train_flag})
...
def test():  
  with tf.Session() as sess:
    ...
    for i in range(1000):
        ...
        x_batch = []
        x_batch.append(img)
        detected = sess.run([prediction], feed_dict={x: x_batch, train_mode: train_flag})
        detected = np.squeeze(detected)
